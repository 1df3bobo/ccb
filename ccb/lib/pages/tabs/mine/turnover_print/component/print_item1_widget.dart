...
class _PrintItem1WidgetState extends State<PrintItem1Widget> {
  ...
  List<String> nameList1 = ['申请账户', '币种', '明细类型', '明细开始日\n期', '明细结速日\n期', ''];
  ...
  void selectIndex(int index) {
    switch (index) {
      ...
      case 3:
        ...
        //   },
        // );
       DateTime? initialDate = DateTime.parse(state.item1title4.replaceAll("/", "-"));
        NewMxPicker.showNewTime(onDateTime: (DateTime date){
          state.item1title4 = DateUtil.formatDate(date, format: "yyyy/MM/dd");
          state.item1title6 = '';
          setState(() {});
        },initialDateTime: initialDate);
      case 4:

        DateTime? initialDate = DateTime.parse(state.item1title5.replaceAll("/", "-"));
        NewMxPicker.showNewTime(onDateTime: (DateTime date){
          state.item1title5 = DateUtil.formatDate(date, format: "yyyy/MM/dd");
          state.item1title6 = '';
          setState(() {});
        },initialDateTime: initialDate);
  ...
  String _contentText(int index) {
    switch (index) {
      case 0:
        return logic.state.item1title1.maskBankCardNumber();
      case 1:
